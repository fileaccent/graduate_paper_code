...
# ncx 改进版(不是论文所用方法,是参考论文所用方法)

def ncx_GIA_opt(circ, tgt, ctrl, anc):
    # circ 量子电路本身
    # tgt 目标量子位
    # ctrl 条件量子位
    # anc 辅助量子位
    n = len(ctrl)
    def process(circ, tgt, ctrl, i):
        # i 位辅助量子位数组的序号
        if (len(ctrl) == 2):
            circ.ccx(ctrl[0], ctrl[1], tgt)
        elif (len(ctrl) == 1):
            circ.cx(ctrl[0], tgt)
        else:
            m = min(int(len(ctrl) / 2) + 1, n - 1)
            frontCtrl = ctrl[:m] # 将ctrl 分成两半
            backCtrl = ctrl[m:]
            backCtrl.append(anc[i])
            # print(len(frontCtrl), len(backCtrl), i)
            circ = process(circ, anc[i], frontCtrl, i + 1)
            circ = process(circ, tgt, backCtrl, i + 1)
            circ = process(circ, anc[i], frontCtrl, i + 1)
            circ = process(circ, tgt, backCtrl, i + 1)
            circ.reset(anc[i])
        return circ
    return process(circ, tgt, ctrl, 0)
...
#         print('\n')
#         print(q_circuit, tgt, ctrl, q_anc)
        q_circuit = ncx_GIA_opt(q_circuit, tgt, ctrl, q_anc)
        # q_circuit.barrier()
...
def walk(n, t):
    ...
    q_anc = QuantumRegister(int(math.log2(n)) + 1, 'anc')
